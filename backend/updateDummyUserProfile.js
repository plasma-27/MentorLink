...
// Function to send PUT request for updating mentor profile
const updateMentorProfile = async (mentorData) => {
  try {
    // PUT request with email in the route to update the profile
    const response = await axios.put(`http://localhost:8000/api/users/profile/mentor/${mentorData.email}`, mentorData);
    console.log(`Mentor ${mentorData.name} profile updated successfully`, response.data);
  } catch (error) {
    console.error(`Error updating mentor ${mentorData.name}:`, error.response ? error.response.data : error.message);
  }
};

// Generate and send requests for updating 50 mentor profiles
for (let i = 0; i < 50; i++) {
